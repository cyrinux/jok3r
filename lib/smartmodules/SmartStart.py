...
class SmartStart:
    ...
    def run(self):
        ...
        # (from Nmap/Shodan)
        processor = MatchstringsProcessor(
            self.service, "service-name-original", self.service.name_original, self.cu
        )
        ...
        processor = MatchstringsProcessor(
            self.service, "banner", self.service.banner, self.cu
        )
    ...
    def start_http(self):
        ...
        if (
            ...
            logger.smartinfo(
                "HTTP authentication (htaccess) detected " "(401 Unauthorized)"
            )
        ...
        if self.service.web_technos:
            ...
            if not self.service.host.os:
                processor = MatchstringsProcessor(
                    self.service, "wappalyzer", self.service.host.os, self.cu
                )
                processor.detect_os()
            ...
            try:
                ...
                logger.debug(
                    'Error when retrieving "web_technos" field ' "from db: {}".format(e)
                )
                technos = list()
