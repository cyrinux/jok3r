...
products_match["http"]["web-server"] = {
    "Apache": {
        ...
        "wig": [WIG_REGEXP.format("Apache"), WIG_REGEXP2.format("Apache")],
    },
    "Hiawatha": {
        "wappalyzer": "Hiawatha",
    ...
    "Lighttpd": {"wappalyzer": "lighttpd", "banner": "lighttpd(\s*[VERSION])?"},
    ...
    "Microsoft/IIS": {
        ...
        "wig": [WIG_REGEXP.format("IIS"), WIG_REGEXP2.format("IIS")],
    },
    "Mongoose": {"banner": "Mongoose httpd(\s*[VERSION])?"},
    "Monkey Http Daemon": {
    ...
    "Nginx": {
        ...
        "wig": [WIG_REGEXP.format("nginx"), WIG_REGEXP2.format("nginx")],
    },
    "Oracle/Http Server": {
        "wappalyzer": "Oracle HTTP Server",
        "banner": "Oracle HTTP Server(\s*(9iAS httpd|Powered by Apache))?(.+\(version [VERSION]\))?",
    },
    "Rejetto/Http File Server": {"banner": "HttpFileServer httpd(\s*[VERSION])?"},
    "Thttpd": {"wappalyzer": "thttpd", "banner": "thttpd(\s*[VERSION])?"},
    "Yaws": {"wappalyzer": "Yaws", "banner": "Yaws httpd(\s*[VERSION])?"},
    "Zeus Web Server": {"banner": "Zeus httpd(\s*Admin Server)?(\s*[VERSION])?"},
}
