...
class ContextUpdater:
    ...
    def add_product(self, type_, name, version):
        self.products.append(Product(type=type_, name=name, version=version))
        ...
        for option in self.specific_options:
            ...
            if match_option:
                if match_option.value == option.value:
                    logger.smartinfo(
                        "Detected option (already known): {name} = "
        ...
        for username in self.usernames:
            ...
            if match_cred:
                if match_cred.password is None:
                    logger.smartinfo(
                        "Detected username (already known): {}".format(username_str)
                    )
                else:
                    logger.smartinfo(
                        "Detected username (password already "
    ...
    def __update_credentials(self):
        ...
        for credential in self.credentials:
            ...
            if match_cred:
                if match_cred.password is None:
                    logger.smartsuccess(
                        "Credentials found (username already "
                        "known): {}".format(credential_str)
                    )
                    match_cred.password = credential.password
                elif match_cred.password != credential.password:
                    logger.smartsuccess(
                        "Credentials found (new password): {}".format(credential_str)
                    )
                    match_cred.password = credential.password
                else:
                    logger.smartinfo(
                        "Credentials detected (already in db): {}".format(
                            credential_str
                        )
                    )
            else:
    ...
    def __update_products(self):
        ...
        for product in self.products:
            product_str = "{type}={name}".format(type=product.type, name=product.name)
            ...
            if match_product:
                ...
                if match_product.name == product.name:
                    ...
                    if product.version:
                        ...
                        if match_product.version == "":
                            logger.smartsuccess(
                                "Version detected for product "
                                "{product}: {version}".format(
                                    product=product_str, version=product.version
                                )
                            )
                    ...
                    # Version not detected
                        ...
                        logger.smartinfo(
                            "Product detected (already in db): "
                ...
                # Different product name detected
                    ...
                    logger.smartsuccess(
                        "Change product {type}: {oldprod} -> "
                        "{newprod}".format(
                            type=product.type, oldprod=oldprod, newprod=newprod
                        )
                    )
            ...
            # Type not present in database
                ...
                logger.smartsuccess(
                    "New product detected: {product} {version}".format(
                        product=product_str, version=product.version
                    )
                )
    ...
    def __update_vulns(self):
        ...
        for vuln in self.vulns:
            ...
            if match_vuln:
                logger.smartinfo(
                    "Detected vulnerability (already in db): {name}".format(
                        name=vuln.name
                    )
                )
            else:
                logger.smartsuccess(
                    "New vulnerability detected: {name}".format(name=vuln.name)
                )
