...
class CredentialsRequester(Requester):
    ...
    def show(self):
        ...
        if not results:
            ...
            for r in results:
                ...
                username = Output.colored(
                    username, color="green" if r.password is not None else "yellow"
                )
                password = {"": "<empty>", None: "<???>"}.get(r.password, r.password)
                password = Output.colored(
                    password, color="green" if r.password is not None else "yellow"
                )
    ...
    def add_cred(self, service_id, username, password, auth_type=None):
        ...
        if cred:
            ...
            service = (
                self.sqlsess.query(Service).filter(Service.id == service_id).first()
            )
            if not service:
                ...
                if cred:
                    if password is None and cred.password is not None:
                        logger.warning(
                            "Credential (username + password) already "
                        ...
                        logger.info(
                            "Username already exists in database, the entry "
                            "is updated."
                        )
                ...
                hostname = (
                    "(" + service.host.hostname + ")" if service.host.hostname else ""
                )
                ...
                logger.success(
                    "Credential {username}/{password}{auth_type} added "
    ...
    def delete(self):
        ...
        if not results:
            ...
            for r in results:
                logger.info(
                    "Credential {username}/{password} from host={ip} "
