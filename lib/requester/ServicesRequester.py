...
class ServicesRequester(Requester):
        ...
        if not results:
            ...
            for r in results:
                ...
                nb_creds = "{}{}{}".format(
                    "{}".format(
                        Output.colored(
                            str(nb_userpass), color="green" if nb_userpass > 0 else None
                        )
                    )
                    if nb_userpass > 0
                    else "",
                    ...
                    "{} usr".format(
                        Output.colored(
                            str(nb_usernames),
                            color="yellow" if nb_usernames > 0 else None,
                        )
                    )
                    if nb_usernames > 0
                    else "",
                ...
                nb_vulns = (
                    Output.colored(
                        str(len(r.vulns)), color="green" if len(r.vulns) > 0 else None
                    )
                    if len(r.vulns) > 0
                    else ""
                )
                ...
                data.append(
                    [
                        ...
                        r.name,
                        StringUtils.wrap(r.banner, 40),
                        StringUtils.wrap(r.url, 40),
    ...
    def add_service(
        ...
        if matching_service:
            ...
            if up:
                ...
                logger.success(
                    "Service added: host {ip} | port {port}/{proto} | "
    ...
    def add_url(
        ...
        matching_service = (
            ...
            .filter(
                (Service.url == url)
                | (Service.url == WebUtils.remove_ending_slash(url))
            )
            .first()
        )
    ...
    def add_target(self, target):
        ...
        matching_service = (
            ...
            .filter(Service.url == target.get_url())
            .first()
        )
        ...
        logger.success(
            "{action}: host {ip} | port {port}/{proto} | "
            "service {service}".format(
        ...
        if not results:
            ...
            for r in results:
                ...
                if not cred:
                    ...
                    logger.success(
                        "Credential {username}/{password}{auth_type} "
    ...
    def delete(self):
        ...
        if not results:
            ...
            for r in results:
                logger.info(
                    "Service {service} host={ip}{hostname} "
                ...
                if len(r.host.services) == 0:
                    logger.info(
                        "Host {ip} {hostname} deleted because it does not "
