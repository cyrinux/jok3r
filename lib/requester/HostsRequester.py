...
class HostsRequester(Requester):
        ...
        if not results:
            ...
            for r in results:
                data.append(
                    [
                        r.ip,
                        StringUtils.wrap(r.hostname, 45)
                        if r.hostname != str(r.ip)
                        else "",
    ...
    def add_or_merge_host(self, host):
        ...
        if match_host:
            ...
            logger.success(
                "Updated host: {ip} {hostname}".format(
                    ip=host.ip,
            ...
            for service in host.services:
                match_service = (
                    ...
                    .filter(Service.url == service.url)
                    .first()
                )
                if match_service:
                ...
                logger.success(
                    "{action} service: host {ip} | port {port}/{proto} | "
        ...
        # If new host, add it in db
            ...
            for service in host.services:
                logger.success(
                    "Added service: host {ip} | port {port}/{proto} | "
                    "service {service}".format(
                        ip=service.host.ip,
                        port=service.port,
                        proto={Protocol.TCP: "tcp", Protocol.UDP: "udp"}.get(
    ...
    def delete(self):
        ...
        if not results:
            ...
            for r in results:
                logger.info(
                    "Host {ip} {hostname} (and its {nb_services} services) "
                    "deleted".format(
                        ip=r.ip,
