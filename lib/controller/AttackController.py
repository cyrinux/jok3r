...
class AttackController(Controller):
    ...
    def __run_for_single_target(self, args):
        ...
        if args.add:
            mission = (
                self.sqlsess.query(Mission).filter(Mission.name == args.add).first()
            )
            if not mission:
                raise AttackException(
                    "The specified mission does not exist in the "
        ...
        reachable = target.smart_check(
            ...
            nmap_banner_grabbing=(
                args.nmap_banner_grab is None or args.nmap_banner_grab == "on"
            ),
        ...
        if mission:
            logger.info(
                "Results from this attack will be saved under mission "
    ...
    def __run_for_multi_targets(self, args):
        ...
        mission = (
            self.sqlsess.query(Mission).filter(Mission.name == args.mission).first()
        )
        if mission:
            logger.info(
            ...
            raise AttackException(
                "Mission {mission} does not exist into the "
        ...
        if args.filters_combined:
            for filt in args.filter:
                logger.info(
                    "Applying filters on mission scope: {filter}".format(filter=filt)
                )
