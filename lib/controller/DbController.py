...
class DbController(cmd2.Cmd):
    ...
    def change_current_mission(self, name, verbose=False):
        ...
        if not mission:
            ...
            self.prompt = (
                Output.colored("jok3rdb", color="light_green", attrs="bold")
    ...
    mission = argparse.ArgumentParser(
        description="Manage missions", formatter_class=formatter_class
    )

    mission_mxg = mission.add_mutually_exclusive_group()
    mission_mxg.add_argument(
        "-a", "--add", action="store", metavar="<name>", help="Add mission"
    )
    mission_mxg.add_argument(
        "-c",
        "--comment",
    ...
    mission_mxg.add_argument(
        "-D", "--reset", action="store_true", help="Delete all missions"
    )
    mission_mxg.add_argument(
        "-r", "--rename", nargs=2, metavar=("<old>", "<new>"), help="Rename mission"
    )
    mission_mxg.add_argument(
        "-S",
        "--search",
    ...
    services_manage = services.add_argument_group(
        "Manage services"
    ).add_mutually_exclusive_group()
    ...
    services_manage.add_argument(
        "-u", "--url", action="store", metavar="<url>", help="Add a new URL"
    )
    services_manage.add_argument(
        "-d",
        "--del",
    ...
    services_manage.add_argument(
        "--https",
        action="store_true",
        help="Switch between HTTPS and HTTP protocol for URL of " "selected service(s)",
    )

    services_creds = services.add_argument_group(
    ...
    services_filters.add_argument(
        "-U", "--up", action="store_true", help="Only show services which are up"
    )
    services_filters.add_argument(
        "-o",
        "--order",
    ...
    services_filters.add_argument(
        "names", nargs="*", metavar="<name1> <name2> ...", help="Services to select"
    )
    ...
    def do_services(self, args):
        ...
        if args.names:
            for n in args.names:
                if not self.settings.services.is_service_supported(n, multi=False):
                    logger.error(
                        "Service {name} is not valid/supported".format(name=n.lower())
                    )
                    print()
                    return
        ...
        if args.search:
            ...
            filter_search.add_condition(
                Condition(args.search, FilterData.COMMENT_SERVICE)
            )
            filter_.add_condition(filter_search)
        ...
        if args.add:
            ...
            if not NetUtils.is_valid_port(port):
                ...
                logger.error(
                    "Service {name} is not valid/supported".format(name=service.lower())
                )
            else:
                req.add_service(
                    host,
        ...
        # --addcred <user> <pass>
            ...
            req.add_cred(
                username=args.addcred[0], password=args.addcred[1], auth_type=None
            )
        ...
        # --addcred-http <user> <pass> <auth-type>
            ...
            if not req.are_only_http_services_selected():
                logger.warning(
                    "Some non-HTTP services are selected. Use --addcred "
            ...
            if not self.settings.services.is_valid_auth_type(args.addcred_http[2]):
                ...
                for auth_type in self.settings.services.get_authentication_types(
                    "http"
                ):
        ...
        # --adduser <user>
            ...
            req.add_cred(username=args.adduser[0], password=None, auth_type=None)

        # --adduser-http <user> <auth-type>
        elif args.adduser_http:
            if not req.are_only_http_services_selected():
                logger.warning(
                    "Some non-HTTP services are selected. Use --adduser "
                    "instead for non-HTTP services"
                )
                print()
                return
            if not self.settings.services.is_valid_auth_type(args.adduser_http[1]):
                ...
                return
            if not req.filter_applied:
                if not self.__confirm_for_all(
                    "add same username for ALL HTTP services"
                ):
                    return

            req.add_cred(
                username=args.adduser_http[0],
                password=None,
    ...
    creds_manage = creds.add_argument_group(
        "Manage credentials"
    ).add_mutually_exclusive_group()
        ...
        if args.service:
            for s in args.service.split(","):
                if not self.settings.services.is_service_supported(s, multi=False):
                    logger.error(
                        "Service {name} is not valid/supported".format(name=s.lower())
                    )
                    print()
                    return
            filter_.add_condition(
                Condition(args.service.split(","), FilterData.SERVICE_EXACT)
            )
        ...
        if args.names:
            for n in args.names:
                if not self.settings.services.is_specific_option_name_supported(n):
                    logger.error(
                        'Option "{name}" is not valid/supported'.format(name=n.lower())
                    )
                    print()
                    return
    ...
    def do_products(self, args):
        ...
        if args.search:
            ...
            filter_search.add_condition(
                Condition(args.search, FilterData.PRODUCT_VERSION)
            )
            filter_.add_condition(filter_search)
        ...
        if args.types:
            for t in args.types:
                if not self.settings.services.is_product_type_supported(t):
                    logger.error(
                        'Product type "{type}" is not valid/supported'.format(
                            type=t.lower()
                        )
                    )
                    print()
    ...
    nmap.add_argument(
        "file", nargs=1, metavar="<xml-results>", help="Nmap XML results file"
    )
    ...
    def do_nmap(self, args):
        ...
        for file in files:
            ...
            if not args.no_http_recheck:
                logger.info(
                    "Each service will be re-checked to detect HTTP services. "
    ...
    def complete_nmap(self, text, line, begidx, endidx):
        ...
        flag_dict = {"nmap": self.path_complete, "-n": self.path_complete}
    ...
    shodan = argparse.ArgumentParser(
        description="Import Shodan host (ips)", formatter_class=formatter_class
    )
    shodan.add_argument(
        "-n",
        "--no-http-recheck",
    ...
    def do_shodan(self, args):
        ...
        if "shodan" not in API_KEYS.keys() or not API_KEYS["shodan"]:
            logger.error(
                'You must add a valid Shodan API key in "apikeys.py" to use '
                "this feature"
            )
            print()
            return
    ...
    file.add_argument(
        "--no-nmap-banner", action="store_true", help="Disable Nmap banner grabbing"
    )
    file.add_argument(
        "file", nargs=1, metavar="<filename>", help="List of targets from a file"
    )
    ...
    results = argparse.ArgumentParser(
        description="Attacks results", formatter_class=formatter_class
    )
    ...
    def do_results(self, args):
        ...
        if (args.service_id or args.check_name) and (args.check_id or args.search):
            logger.error(
                "--service-id|--check-name and --check-id|--search are "
                "mutually exclusive"
            )
            print()
            return
        ...
        if (
            not args.service_id
            and not args.check_name
            and not args.check_id
            and not args.search
        ):
    ...
    report.add_argument(
        "--no-screen", action="store_true", help="Disable not take web page screenshots"
    )
    report.add_argument(
        "path",
        nargs="?",
    ...
    def __confirm_for_all(self, action):
        ...
        :rtype: bool
        """
        if not Output.prompt_confirm(
            "No filter applied. Are you sure you "
