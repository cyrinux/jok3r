    ...
    def parse(self, http_recheck=True):
        ...
        for ip in self.ips_list:
            ...
            try:
                ...
                logger.error(
                    "Error when querying shodan for IP {ip}: {exc}".format(ip=ip, exc=e)
                )
                return None

            logger.info(
                "Importing Shodan results from https://www.shodan.io/host/"
                "{ip}".format(ip=ip)
            )
            ...
            logger.info(
                "[Host {current_host}/{total_host}] Parsing host: "
                "{ip}{hostname} ...".format(
