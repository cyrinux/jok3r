...
class Reporter:
    ...
    def run(self):
        ...
        dirname = "{mission}-{datetime}".format(
            mission=StringUtils.clean(
                self.mission.replace(" ", "_"), allowed_specials=("_", "-")
            ),
        ...
        if not FileUtils.create_directory(self.output_path):
            logger.error(
                'Unable to create report directory: "{path}"'.format(
        ...
        if not FileUtils.create_directory(screens_dir):
            ...
            for service in services:
                if (
                    service.name == "http"
                    and service.screenshot is not None
                    and service.screenshot.status == ScreenStatus.OK
                ):

                    img_name = "scren-{ip}-{port}-{id}".format(
                        ip=str(service.host.ip), port=service.port, id=service.id
                    )
                    path = screens_dir + "/" + img_name

                    ImageUtils.save_image(service.screenshot.image, path + ".png")
        ...
        for service in services:
            ...
            if FileUtils.write(self.output_path + "/" + filename, html):
                logger.info("{filename} file generated".format(filename=filename))
            else:
                logger.error(
        ...
        logger.success(
            "HTML Report written with success in: {path}".format(path=self.output_path)
        )
        logger.info(
            "Important: If running from Docker container, make sure to run "
            '"xhost +" on the host before'
        )
        if Output.prompt_confirm(
            "Would you like to open the report now ?", default=True
        ):
    ...
    def __generate_index(self):
        ...
        tpl = tpl.replace(
            "{{TABLE_SERVICES_CONTENT}}", self.__generate_table_services()
        )
        ...
        tpl = tpl.replace(
            "{{TABLE_PRODUCTS_CONTENT}}", self.__generate_table_products()
        )
        tpl = tpl.replace(
            "{{TABLE_CREDS_CONTENT}}", self.__generate_table_credentials()
        )
        ...
        if len(services) == 0:
            ...
            for service in services:
                ...
                hostname = (
                    ...
                    if service.host.ip != service.host.hostname
                    else ""
                )
                ...
                nb_creds = "{}{}{}".format(
                    ...
                    '<span class="text-yellow">{}</span>'.format(str(nb_usernames))
                    if nb_usernames > 0
                    else "",
                )
                # if nb_creds == '':
                ...
                # Encrypted ? (SSL/TLS)
                enc = (
                    '<span class="mdi mdi-lock" title="SSL/TLS encrypted"></span>'
                ...
                if service.name == "http":
                    ...
                    for t in product_types:
                        ...
                        if product:
                            technos += (
                                '<span class="badge badge-{type} badge-light">'
                    ...
                    for p in service.products:
                        technos += (
                            '<span class="badge badge-generic badge-light">'
                            "{name}{version}</span>".format(
                                type=p.type,
                                name=p.name,
    ...
    def __generate_table_web(self):
        ...
        if len(services) == 0:
            ...
            for service in services:
                ...
                if product:
                    waf = (
                        '<span class="badge badge-web-application-firewall '
    ...
    def __generate_results_page(self, service):
        ...
        tpl = tpl.replace("{{MISSION_NAME}}", self.mission)
        tpl = tpl.replace(
            "{{SERVICE_ICON}}", IconsMapping.get_icon_html("service", service.name)
        )
    ...
    def __generate_command_outputs(self, service):
        ...
        for r in results:
            ...
            for o in r.command_outputs:
                ...
                m = re.search(
                    '<pre class="ansi2html-content">(?P<output>.*)' "</pre>\n</body>",
                    output,
                    re.DOTALL,
                )
                if m:
                    ...
                    <pre>{output}</pre>
                    """.format(
                        cmdline=o.cmdline, output=output
                    )
