...
class Tool:
    def __init__(
        ...
        self.tool_dir = (
            FileUtils.absolute_path(
                "{toolbox}/{service}/{name}".format(
                    toolbox=TOOLBOX_DIR, service=self.target_service, name=self.name
                )
            )
    ...
    def remove(self, settings):
        ...
        if self.install_command:
            if not FileUtils.is_dir(self.tool_dir):
                logger.warning(
                    'Directory "{dir}" does not exist'.format(dir=self.tool_dir)
                )
                return False
        ...
        if settings.change_installed_status(
            self.target_service, self.name, install_status=False
        ):
            logger.success("Tool marked as uninstalled")
        else:
            logger.error(
                "An unexpected error occured when trying to mark the tool "
                "as uninstalled !"
            )
            return False
    ...
    def __create_tool_dir(self):
        ...
        if self.tool_dir:
            if FileUtils.is_dir(self.tool_dir):
                logger.info(
                    'Directory "{dir}" already exists'.format(dir=self.tool_dir)
                )
                return True
    ...
    def __check_pre_install(self, settings, fast_mode=False):
        ...
        if self.installed:
            logger.info(
                "{tool} is already installed (according to settings), "
            ...
            logger.warning(
                "The tool {tool} has no installation command specified in "
                "config file".format(tool=self.name)
            )

            if fast_mode or Output.prompt_confirm(
                "Is the tool already installed on your system ?", default=True
            ):

                try:
                    if settings.change_installed_status(
                        self.target_service, self.name, True
                    ):

                        logger.success(
                            "Tool {tool} has been marked as installed in "
                ...
                logger.info(
                    "Tool {tool} is still not marked as installed in "
                    "settings".format(tool=self.name)
                )
            return False
    ...
    def __check_pre_update(self, settings, fast_mode=False):
        ...
        if not self.installed:
            logger.info(
                "{tool} is not installed yet (according to settings), "
                "skipped".format(tool=self.name)
            )
            return False
        ...
        if self.install_command and not FileUtils.is_dir(self.tool_dir):
            logger.warning(
                "Tool directory does not exist but tool marked as "
    ...
    def __run_install_update(self, fast_mode, update=False):
        ...
        """
        if update:
            cmd = self.update_command.get_cmdline(self.tool_dir)
        else:
            cmd = self.install_command.get_cmdline(self.tool_dir)
    ...
    def __check_post_install_update(self, settings, fast_mode=False, update=False):
        ...
        if not fast_mode:
            if not self.check_command:
                logger.info(
                    "No check_command defined in settings for {tool}, will "
                    "assume it is correctly {mode}".format(tool=self.name, mode=mode[1])
                )
            else:
                logger.info(
                    "Now, checking if {tool} has been {mode} correctly. "
        ...
        if status:
            try:
                ...
                if settings.change_installed_status(
                    self.target_service, self.name, install_status=True
                ):

                    logger.success(
                        "Tool {tool} has been marked as successfully "
                        "{mode}".format(tool=self.name, mode=mode[1])
                    )
                    return True
                else:
                    logger.error(
                        "Error when updating configuration file "
                        '"{filename}{ext}"'.format(
                            filename=INSTALL_STATUS_CONF_FILE, ext=CONF_EXT
                        )
                    )
                ...
                logger.error(
                    "An unexpected error occured when trying to mark the "
            ...
            logger.warning(
                "Tool {tool} has not been marked as {mode}".format(
    ...
    def __run_check_command(self):
        ...
        """
        logger.info(
            "Running the check command for the tool {tool}...".format(tool=self.name)
        )
        ...
        return Output.prompt_confirm(
            "Does the tool {tool} seem to be running "
            "correctly ?".format(tool=self.name),
            default=True,
        )
