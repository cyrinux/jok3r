...
class ArgumentsParser:
    ...
    def check_args(self):
        ...
        elif self.mode == Mode.INFO:
            return self.check_args_info()
    ...
    def check_args_toolbox(self):
        ...
        service = (
            self.args.show_toolbox_for_svc
        ...
        tool = (
            self.args.install_tool or self.args.update_tool or self.args.uninstall_tool
        )
        ...
        if tool:
            if self.settings.toolbox.get_tool(tool) is None:
                ...
                logger.error(
                    'Tool "{tool}" is not referenced inside the toolbox. '
    ...
    def check_args_info(self):
        ...
        service = self.args.show_checks or self.args.show_attack_profiles
        ...
        if service and not self.settings.services.is_service_supported(
            service, multi=False
        ):
    ...
    def __check_args_attack_single_target(self):
        ...
        if target.lower().startswith("http://") or target.lower().startswith(
            ...
            if self.args.service and self.args.service.lower() != "http":
                logger.warning(
                    "URL only supported for HTTP service. "
        ...
        # Target specified is IP[:PORT] or HOSTNAME[:PORT]
            ...
            self.args.target_mode = (
                TargetMode.IP
            )  # Actually can be either IP or Hostname
            ...
            if len(s) == 2:
                ...
                if not (0 <= self.args.target_port <= 65535):
                    logger.error(
                        "Target port is not valid. Must be in the " "range [0-65535]"
                    )
                    return False

            elif len(s) > 2:
                logger.error(
                    "Incorrect target format. Must be either IP[:PORT] or " "an URL"
                )
                return False
            ...
            if self.args.service:
                ...
                if not self.args.target_port:
                    ...
                    if self.args.target_port:
                        logger.info(
                            "Default port for service {service} will be used: "
                            "{port}/{proto}".format(
                        ...
                        logger.error(
                            "Target port is not specified and no default port"
            ...
            # Try to get default service for provided port if not specified
                ...
                if not self.args.target_port:
                    ...
                    if not self.args.service:
                        logger.error(
                            "Cannot automatically determine the target "
                    ...
                    logger.info(
                        "Automatic service detection based on target port: "
            ...
            for c in self.args.filter:
                ...
                for cond in c.split(";"):
                    ...
                    if name not in TARGET_FILTERS.keys():
                        logger.error(
                            "Filter option {filter} is not supported. "
    ...
    def __check_args_attack_selection(self):
        ...
        # Attack profile
            ...
            if not profile:
                logger.error(
                    "Attack profile {profile} does not exist. "
                    'Check "info --attack-profiles'.format(profile=self.args.profile)
                )
                return False

            elif self.args.target_ip_or_url and not profile.is_service_supported(
                self.args.service
            ):

                logger.error(
                    "Attack profile {profile} is not supported for "
        ...
        for f in self.args.userlist, self.args.passlist:
            if f:
                if not FileUtils.can_read(f):
                    logger.error(
                        "File {file} does not exist or cannot be read".format(file=f)
                    )
                    return False
    ...
    def __check_args_attack_context_cred(self):
        ...
        for cred in self.args.creds:
            current_cred = Credential(
                type="", username="", password=None  # relevant for HTTP
            )
            ...
            if len(cred) == 3:
                if "." in cred[0]:
                    ...
                    if svc != "http":
                        logger.error(
                            "Auth-type in --cred is only supported with "
                        ...
                        logger.error(
                            "Invalid authentication type provided in --cred. "
                ...
                if self.args.service and self.args.service != svc:
                    logger.error(
                        "Single target mode selected but targeted service "
            ...
            # Accepted only in single target mode
                ...
                if self.args.service:
                    ...
                    logger.error(
                        "Service must be specified in --cred in multi targets"
        ...
        for user in self.args.users:
            ...
            if len(user) == 2:
                if "." in user[0]:
                    ...
                    if svc != "http":
                        logger.error(
                            "Auth-type in --user is only supported with "
                        ...
                        logger.error(
                            "Invalid authentication type provided in --user. "
                            'Check "info --list-http-auth".'
                        )
                        return False
            ...
            # Accepted only in single target mode
                ...
                if self.args.service:
                    ...
                    logger.error(
                        "Service must be specified in --user in multi targets"
        ...
        for product in self.args.products:
            if "=" in product:
                ...
                if not self.settings.services.is_product_type_supported(
                    ...
                    logger.error(
                        'The product type "{type}" submitted is not '
                    ...
                    logger.error(
                        'The product name "{name}" submitted for '
                    ...
                    logger.error(
                        'Same product type "{type}" is defined several '
                    ...
                    products[service].append(Product(type=type_, name=name))
        ...
        for option in self.args.options:
            if "=" in option:
                ...
                if not self.settings.services.is_specific_option_name_supported(
                    ...
                    logger.error(
                        'The specific option name "{name}" is not '
                    ...
                    logger.error(
                        'The value for the specific option named "{name}" is '
                    ...
                    options[service].append(Option(name=name, value=value))
            else:
