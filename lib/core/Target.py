...
class Target:
    ...
    def __init__(self, service, services_config):
        ...
        if (
            not self.service.host.ip
            or not self.service.host.hostname
    ...
    def __init_with_ip(self):
        ...
        if NetUtils.is_valid_ip(self.service.host.ip):
            ...
            if self.service.host.ip:
                logger.info(
                    "DNS lookup on {hostname} -> IP: {ip}".format(
                        hostname=self.service.host.hostname, ip=self.service.host.ip
                    )
                )
            else:
                raise TargetException(
                    "Unable to resolve {}".format(self.service.host.hostname)
                )
    ...
    def get_protocol(self):
        proto = {Protocol.TCP: "tcp", Protocol.UDP: "udp"}
    ...
    def smart_check(
        ...
        if reverse_dns_lookup:
            if (
                ...
                logger.info(
                    "Reverse DNS lookup for {ip}...".format(
        ...
        # For HTTP, also grab HTML title and HTTP response headers
            ...
            html_title_grabbing or availability_check
        ...
        if (
            ...
            logger.info(
                "Grab service info for [{service}] via Nmap...".format(service=self)
            )
            self.__run_nmap()
    ...
    def __reverse_dns_lookup(self):
        ...
        if hostname != self.service.host.ip:
            logger.info(
                "{ip} -> {hostname}".format(ip=self.service.host.ip, hostname=hostname)
            )
        else:
    ...
    def __grab_html_title_and_headers(self):
        ...
        if self.service.url:
            ...
            try:
                ...
                if (
                    ...
                    new_url = WebUtils.replace_hostname_by_ip(
                        self.service.url, self.service.host.ip, self.service.port
                    )
                    print(new_url)
            ...
            if is_reachable:
                if resp_headers:
                    self.service.http_headers = "\n".join(
                        "{}: {}".format(key, val) for (key, val) in resp_headers.items()
                    )
                else:
                    self.service.http_headers = ""
    ...
    def __run_nmap(self):
        ...
        if nmap_info["os"]:
            if not self.service.host.os:
                ...
                logger.info("Detected OS has changed = {os}".format(os=nmap_info["os"]))
    ...
    def print_context(self):
        ...
        if self.get_os():
            logger.info(
                "OS type detected for this target: {os}".format(os=self.get_os())
            )
