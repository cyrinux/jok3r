...
class AttackScope:
    ...
    def attack(self):
        ...
        attack_progress = manager.counter(
            ...
            counter_format=STATUSBAR_FORMAT_SINGLE,
        )  # For single target
    ...
    def show_summary(self):
        ...
        data = list()
        columns = ["id", "IP", "Hostname", "Port", "Proto", "Service", "Banner", "URL"]
        id_ = 1
        for target in self.targets:
            ...
            data.append(
                [
                    ...
                    Output.colored(
                        target.get_ip(), color=pointer_color, attrs=pointer_attr
                    ),
                    Output.colored(
                        StringUtils.wrap(target.get_host(), 50),
                        color=pointer_color,
                        attrs=pointer_attr,
                    ),
                    Output.colored(
                        str(target.get_port()), color=pointer_color, attrs=pointer_attr
                    ),
                    Output.colored(
                        target.get_protocol(), color=pointer_color, attrs=pointer_attr
                    ),
                    Output.colored(
                        target.get_service_name(),
                        color=pointer_color,
                    ...
                    Output.colored(
                        StringUtils.wrap(target.get_banner(), 55),
                        color=pointer_color,
                        attrs=pointer_attr,
                    ),
                    Output.colored(
                        StringUtils.wrap(target.get_url(), 50),
                        color=pointer_color,
                        attrs=pointer_attr,
                    ),
                ]
        ...
        if len(self.targets) > ATTACK_SUMMARY_TABLE_MAX_SIZE:
            logger.info(
                "Table has been truncated. Total number of loaded "
