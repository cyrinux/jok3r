...
class Settings:
    ...
    def __init__(self):
        ...
        if not FileUtils.is_dir(SETTINGS_DIR):
            raise SettingsException(
                "Configuration directory ({dir}) does not "
        ...
        if TOOLBOX_CONF_FILE + CONF_EXT not in files:
            raise SettingsException(
                "Missing mandatory {toolbox}{ext} settings "
        ...
        if ATTACK_PROFILES_CONF_FILE + CONF_EXT not in files:
            raise SettingsException(
                "Missing mandatory {profiles}{ext} settings "
                'file in directory "{dir}"'.format(
                    profiles=ATTACK_PROFILES_CONF_FILE, ext=CONF_EXT, dir=SETTINGS_DIR
                )
            )
        ...
        if INSTALL_STATUS_CONF_FILE + CONF_EXT not in files:
            ...
            logger.info(
                "{status}{ext} settings file created in directory "
                '"{dir}"'.format(
        ...
        for section in self.config_parsers[TOOLBOX_CONF_FILE].sections():
            ...
            if newtool is not None:
                if not self.toolbox.add_tool(newtool):
                    logger.warning(
                        '[{filename}{ext} | Section "{section}"] Unable '
    ...
    def __parse_tool_options(self, section, tool_config):
        ...
        for opt in TOOL_OPTIONS[MANDATORY]:
            if opt not in optparsed:
                logger.warning(
                    '{prefix} Missing mandatory option "{option}", '
        ...
        for opt in optparsed:
            ...
            if opt not in TOOL_OPTIONS[MANDATORY] + TOOL_OPTIONS[OPTIONAL]:
                logger.warning(
                    '{prefix} Option "{option}" is not supported, '
            ...
            if opt == "name":
                ...
                if tool_config[opt] not in self.services.list_services(multi=True):
                    logger.warning(
                        '{prefix} Service specified in "target_service" is '
            ...
            if opt in TOOL_OPTIONS[MANDATORY] and not tool_config[opt]:
                logger.warning(
                    '{prefix} Mandatory option "{option}" is empty, tool '
    ...
    def __parse_tool_install_status(self, tool_config):
        ...
        """
        is_installed = (
            self.config_parsers[INSTALL_STATUS_CONF_FILE]
            .safe_get(tool_config["target_service"], tool_config["name"], "false", None)
            .lower()
            .strip()
        )
    ...
    def __parse_section_config(self, service, service_config):
        ...
        if default_port is None or default_port < 0 or default_port > 65535:
            raise SettingsException(
                '{prefix} Invalid value for option "default_port",'
        ...
        categories = list(
            map(
                lambda x: StringUtils.clean(x.lower(), allowed_specials=("-", "_")),
        ...
        if not categories:
            raise SettingsException(
                '{prefix} Option "categories" must have at least '
    ...
    def __parse_section_supported_list_options(self, service, service_config):
        ...
        for opt in options_list:
            ...
            if "supported_" + opt not in optparsed:
                raise SettingsException(
                    '{prefix} No option "supported_{option}" '
            ...
            if not values:
                raise SettingsException(
                    '{prefix} Option "supported_{option}" is '
    ...
    def __parse_section_products(self, service, service_config):
        ...
        for product_type in optparsed:
            ...
            product_type = StringUtils.clean(
                product_type.lower(), allowed_specials=("-", "_")
            )
    ...
    def __parse_check_section(self, service, section, check_config):
        ...
        for opt in CHECK_OPTIONS[MANDATORY]:
            if opt not in optparsed:
                logger.warning(
                    '{prefix} Missing mandatory option "{option}", the '
        ...
        for opt in optparsed:
            ...
            # Check for unsupported options
            if (
                opt not in CHECK_OPTIONS[MANDATORY] + CHECK_OPTIONS[OPTIONAL]
                and not opt.startswith("command_")
                and not opt.startswith("context_")
            ):
                logger.warning(
                    '{prefix} Option "{option}" is not supported, the '
                    "check is ignored".format(prefix=log_prefix, option=opt)
                )
                continue
            ...
            if opt == "name":
                ...
                if check_config[opt] not in self.services[service]["checks"].categories:
                    logger.warning(
                        '{prefix} Category "{category}" is not supported, '
                ...
                if tool is None:
                    logger.warning(
                        '{prefix} The tool "{tool}" does not exist, the '
                ...
                if val not in API_KEYS.keys():
                    logger.warning(
                        '{prefix} API key "{apikey}" is not supported in '
            ...
            if opt in CHECK_OPTIONS[MANDATORY] and not check_config[opt]:
                logger.warning(
                    '{prefix} Mandatory option "{option}" is empty, the '
                    "check is ignored".format(prefix=log_prefix, option=opt)
                )
                return False
    ...
    def __parse_commands(self, service, section):
        ...
        for cmd in cmdlines:
            ...
            if context_requirements is None:
                logger.warning(
                    "{prefix} Context requirements are invalid, the check "
        ...
        if not commands:
            logger.warning(
                "{prefix} No command is specified, the check is "
    ...
    def __parse_context_requirements(self, service, section, num_context, context_str):
        ...
        context_str = (
            context_str.replace("NO_AUTH", str(NO_AUTH))
        ...
        try:
            ...
            logger.warning(
                "{prefix} Parsing error. Valid dictionary syntax is "
        ...
        for cond, val in context.items():
            ...
            if cond == "auth_status":
                if val not in (NO_AUTH, USER_ONLY, POST_AUTH, None):
                    logger.warning(
                        '{prefix} Invalid value for "auth_status" '
            ...
            # OS
                ...
                if not val:
                    logger.warning(
                        '{prefix} "os" context requirement is specified'
            ...
            # Specific option
                ...
                if type_ == OptionType.LIST:
                    if val == "undefined":
                        ...
                        sup_vals = self.services[service]["supported_list_options"][
                            cond
                        ]
                        for e in val:
                            if e not in sup_vals:
                                logger.warning(
                                    "{prefix} Context requirement "
                                    '"{option}" contains an invalid element '
                                    '("{element}")'.format(
                                        prefix=log_prefix, option=cond, element=e
                                    )
                                )
                                return None
                ...
                # Context requirement must be boolean or None
                    ...
                    if not isinstance(val, bool):
                        logger.warning(
                            '{prefix} Context requirement "{option}" must '
                ...
                if val in ("undefined", "any", "any|version_known"):
                    ...
                    for e in val:
                        ...
                        if product_name.lower() not in list(
                            ...
                            logger.warning(
                                '{prefix} Context requirement "{option}" '
                                'contains an invalid product ("{product}")'.format(
                                    prefix=log_prefix, option=cond, product=product_name
                                )
                            )
                        req_products[cond] = val
            ...
            # Not supported
                ...
                logger.warning(
                    '{prefix} Context requirement "{option}" is not '
        ...
        return ContextRequirements(
            specific_options=req_specific_options,
    ...
    def __create_attack_profile(self, section):
        ...
        for opt in optparsed:
            ...
            # correspond to the name of the service)
                ...
                list_checks = self.config_parsers[
                    ATTACK_PROFILES_CONF_FILE
                ].safe_get_list(section, opt, ",", [])

                if not list_checks:
                    logger.warning(
                        "{prefix} List of checks for {service} is empty, "
                ...
                for c in list_checks:
                    if not self.services.get_service_checks(opt).is_existing_check(c):
                        logger.warning(
                            '{prefix} The check "{check}" does not exist '
            ...
            # Unsupported option
                ...
                logger.warning(
                    '{prefix} The option "{option}" is not supported '
        ...
        if not description:
            logger.warning(
                "{prefix} A description must be given for the attack "
    ...
    def change_installed_status(self, target_service, tool_name, install_status):
        ...
        if not parser.safe_set(target_service, tool_name, value):
            raise SettingsException(
                "Unable to change install status value for the "
    ...
    def save(self, conf_filename):
        ...
        try:
            ...
            logger.error(
                "Error occured when saving changes in settings file "
