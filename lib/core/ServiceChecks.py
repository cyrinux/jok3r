...
class ServiceChecks:
    ...
    def run(
        ...
        if filter_checks is None and attack_profile is None:
            self.__run_standard_mode(
                target, arguments, sqlsession, filter_categories, attack_progress
            )
    ...
    def __run_standard_mode(
        self, target, arguments, sqlsession, filter_categories, attack_progress=None
    ):
        """
        ...
        checks_progress = manager.counter(
            total=nb_checks + 1,
            desc="",
            unit="check",
            leave=False,
        ...
        for category in self.categories:
            ...
            for check in self.checks[category]:
                ...
                status = (
                    " +--> Current check [{cur}/{total}]: {category} > "
                    "{checkname}".format(
                        cur=j,
                        total=nb_checks,
                ...
                if result is None or arguments.args.recheck == True:
                    ...
                    if check.check_target_compliance(target):
                        Output.title2(
                            "[{category}][Check {num:02}/{total:02}] "
                        ...
                        if not check.tool.installed:
                            ...
                            try:
                                check.run(target, arguments, sqlsession)

                            except KeyboardInterrupt:
                                print()
                                logger.warning(
                                    "Check {check} skipped !".format(check=check.name)
                                )
    ...
    def __run_special_mode(
        ...
        if filter_checks:
            filter_checks = list(
                filter(lambda x: self.is_existing_check(x), filter_checks)
            )

            if not filter_checks:
                logger.warning(
                    "None of the selected checks is existing for the "
        ...
        # User has submitted an attack profile
            ...
            if not attack_profile.is_service_supported(target.get_service_name()):
                logger.warning(
                    "The attack profile {profile} is not supported for "
        ...
        for checkname in filter_checks:
            ...
            results_req.select_mission(target.service.host.mission.name)
            filter_ = Filter(FilterOperator.AND)
            filter_.add_condition(Condition(target.service.id, FilterData.SERVICE_ID))
            filter_.add_condition(Condition(check.name, FilterData.CHECK_NAME))
            results_req.add_filter(filter_)
            result = results_req.get_first_result()
            ...
            if result is None or arguments.args.recheck == True:
                ...
                if check.check_target_compliance(target):
                    ...
                    logger.info(
                        "[Check {num:02}/{total:02}] "
                        "{name} > Skipped because context requirements are "
                        "not matching the target".format(
                            name=check.name, num=i, total=len(filter_checks)
                        )
                    )
                    time.sleep(0.2)
                ...
                logger.info(
                    "[Check {num:02}/{total:02}] "
                    "{name} > Skipped because the check has already "
                    "been run".format(name=check.name, num=i, total=len(filter_checks))
                )
                time.sleep(0.2)
