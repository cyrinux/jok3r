...
class Output(object):
    ...
    def prompt_confirm(question, default=None):
        ...
        return humanfriendly.prompts.prompt_for_confirmation(
            colored.stylize("[?] ", colored.fg("cyan") + colored.attr("bold"))
            + question,
            default=default,
            padding=False,
        )
    ...
    def prompt_choice(question, choices, default=None):
        ...
        while True:
            ...
            if ret.lower() in choices:
                return ret.lower()
            else:
                valid = " / ".join(
                    "{} = {}".format(key, val) for key, val in choices.items()
                )
    ...
    def prompt_choice_range(question, mini, maxi, default):
        ...
        while True:
            try:
                ret = int(
                    humanfriendly.prompts.prompt_for_input(
                        colored.stylize(
                            "\b[?] ", colored.fg("cyan") + colored.attr("bold")
                        )
                        + question,
                        default=default,
                    )
            ...
            if mini <= ret <= maxi:
                ...
                logger.warning(
                    "Invalid value. Valid values are in range "
    ...
    def prompt_choice_verbose(choices, default=None):
        ...
        return humanfriendly.prompts.prompt_for_choice(
            choices, default=default, padding=False
        )
