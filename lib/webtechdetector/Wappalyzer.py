...
class Wappalyzer(requests.Session):
    def __init__(self, url, filename="apps.json"):
        ...
        self.headers["User-Agent"] = (
            "Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko) " "Chrome/65.0.3325.181"
        )
        ...
        self.data.scripts = [
            script["src"]
            for script in BeautifulSoup(self.data.text, "html.parser").find_all(
                "script", {"src": True}
            )
        ]
    ...
    def parsePatterns(self, patterns):
        ...
        if isinstance(patterns, str) or isinstance(patterns, list):
            patterns = {"main": self.asArray(patterns)}
                ...
                if app and app.props.implies:
                    for implied in self.asArray(app.props.implies):
                        ...
                        for id in app.confidence:
                            ...
                            apps[implied["string"]].confidence[ind] = app.confidence[
                                id
                            ] * int(
    ...
    def analyzeMeta(self, app, html):
        ...
        for match in matches:
            for meta in patterns:
                r = re.search(
                    "(?:name|property)=[\"']" + meta + "[\"']", match, re.IGNORECASE
                )
                if r:
                    content = re.findall(
                        "content=[\"|']([^\"']+)[\"|']", match, re.IGNORECASE
                    )
                    for pattern in patterns[meta]:
                        if content and re.search(
                            pattern["regex"], content[0], re.IGNORECASE
                        ):
                ...
                for i in range(len(matches)):
                    ...
                    if ternary and len(ternary) >= 3:
                        version = version.replace(
                            ternary[0], ternary[1] if match else ternary[2]
                        )
...
def getSimple(url):
    ...
    for appName, app in apps.items():
        ...
        for category_id in categories:
            category_name = (
                wappalyzer.db["categories"][str(category_id)]["name"]
                .lower()
                .replace(" ", "-")
            )
